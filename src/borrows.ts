...
import * as csv_writer from 'csv-write-stream'
...
class APYS{
  apys: Map<number, Map<string, Object>>
  constructor(){
    this.apys = new Map<number, Map<string, Object>>();
  }

  get(token:string, date:number){
    let data = (this.apys.get(date))?.get(token)
    if(data == undefined){
  ...
  async load_apy_from_file(filename){
    ...
    const stream = fs.createReadStream(filename, { encoding: "utf-8" })
      ...
      .on("data", (chunk) => {
        ...
        if(retrieved_data == undefined){
          retrieved_data = new Map<string, Object>();
...
type Borrow = {date: number, amount: number}
...
class Borrower{
  ...
  constructor(token: string, borrows: Array<Borrow>, account: string){
    this.token = token
  ...
  add_borrow(amount: number, date: number){
    // int to date
  ...
  repay_borrow(amount: number, date: number, apys: APYS){
    amount = Number(amount)
    const start_amount = amount
    const real_date = new Date(date*1000)
    const norm_date = new Date(real_date.getFullYear(), real_date.getMonth(), real_date.getDate()).getTime()
    let cumulative_apy:number  = 0
    let start_date: number = norm_date
    let end_date: number = norm_date
    ...
    let apy: number = Number(data["Borrow APY"])
    ...
    while(amount > 0){
      ...
      const real_borrow: number = apy*years_between + borrow.amount
...
class Queue {
  ...
  async pop(): Promise<any> {
    if (this.queue.length == 0) {
      const new_queue = await this.getNextQueue();
  ...
  async getNextQueue(): Promise<any[]> {
    const vars = {
      first: this.size,
      skip: this.skip,
...
class EventQueue{
  ...
  async pop(): Promise<any> {
    const peeks = await Promise.all(this.queues.map( async (q) => ((await q.peek())?.blockNumber) ?? Infinity))
...
async function run() {
  ...
  while((event = await eventQueue.pop()) != undefined && event.blockTime <= lastBlockTime) {
